...
//                }
                ...
                Log.e(MainActivity.TAG,"getSiteList = ${response.body()}")
...
//                }
    ...
    fun checkDuplicatedSiteName(siteName: String) {
        viewModelScope.launch {
            try {
                val response: Response<ResponseResult> = remoteRepository.checkDuplicatedSiteName(siteName)
    ...
    fun registerSite(mode: Mode, siteId:String, siteName:String, siteAddr:String, descr:String){
        ...
        viewModelScope.launch {
            try {
                val response: Response<ResponseResult> = remoteRepository.registerSite(command,siteId,siteName,siteAddr,descr)
                response.body()?.let { response ->
